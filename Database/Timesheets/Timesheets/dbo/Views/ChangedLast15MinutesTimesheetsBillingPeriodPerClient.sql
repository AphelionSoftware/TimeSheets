CREATE VIEW [dbo].[ChangedLast15MinutesTimesheetsBillingPeriodPerClient]
as
...
WHERE  --DATEDIFF(mi, TD.LastUpdateDate, getdate() ) < 15
(TD.LastUpdateDate >= ISNULL((SELECT MAX(loadtimes.ExportClientTime) from config.LoadTimes), '2013/04/01'))
GROUP BY ddMonth.BillingYear, ddMonth.BillingPeriodText,c.ClientName